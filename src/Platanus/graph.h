...
public:
    ...
    void printContig(const std::string &outputFilename, const double coverageRatio) const;
    ...
    void cutBranchIterative(const unsigned long long numThread);
...
unsigned long long BruijnGraph<KMER>::crushBubble(const double averageCoverage)
{
    ...
    const unsigned short coverageThreshold = static_cast<unsigned short>(std::min(static_cast<u64_t>(averageCoverage * BUBBLE_COVERAGE_RATE + 0.5), static_cast<u64_t>(UINT16_MAX)));
...
//////////////////////////////////////////////////////////////////////////////////////
template <typename KMER>
void BruijnGraph<KMER>::printContig(const std::string &outputFilename, const double coverageRatio) const
{
    std::ofstream out(outputFilename.c_str());
    ...
    for (auto it = leftStraightTable.begin(), end = leftStraightTable.end(); it != end; ++it) {
        ...
        if (straight.coverage == UINT16_MAX) continue;
        ++numSeq;
        out << ">seq" << numSeq << "_len" << straight.length + kmerLength - 1 << "_cov" << static_cast<unsigned short>(straight.coverage * coverageRatio + 0.5) << "\n";

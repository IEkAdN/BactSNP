...
void split(vector<string>& s_sp, const string& s, const string& delim,
  ...
  while ((pos_elemEnd = s.find_first_of(delim, pos_elemBeg))
    ...
    if (token_compress) {
      ...
      // the empty string is not pushed back to s_sp.
  ...
  if (token_compress) {
    // If the last character of s is one of delim (pos_elemBeg == s.length()),
    // the empty string is not pushed back to s_sp.
    if (pos_elemBeg < s.length()) {
      s_sp.push_back(s.substr(pos_elemBeg, std::string::npos));
    }
  } else {
    s_sp.push_back(s.substr(pos_elemBeg, std::string::npos));
  }

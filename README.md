...
BactSNP is currently available only on Linux. Users can download BactSNP as a binary package, a source package, or a source RPM package. The binary package can be used without compiling, but they are not available for all platforms. The source package needs compiling. The source RPM can be compiled and installed using RPM. All packages are available at https://github.com/IEkAdN/BactSNP/releases.
